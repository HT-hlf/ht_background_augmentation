...
# 数据集路径
dataset_image_path=r'F:\doing\Motor_detection_dataset\second_dataset\labeled\second_yolo_augementation1\images'
dataset_label_path=r'F:\doing\Motor_detection_dataset\second_dataset\labeled\second_yolo_augementation1\labels'
...
for i in range(len(dataset_image_list)):
    ...
    if random.randint(0,1):
        background_image_resize = cv2.flip(background_image_resize, 1, dst=None)
    else:
        background_image_resize = cv2.flip(background_image_resize, 2, dst=None)
    ...
    if random.randint(0,1):
        dataset_image_cv=SaltAndPepper(dataset_image_cv,random.uniform(0.01,0.03))
    else:
        dataset_image_cv =  addGaussianNoise(dataset_image_cv,random.uniform(0.01,0.03))
    if random.randint(0, 1):
        dataset_image_cv = darker(dataset_image_cv,random.uniform(0.85,0.98))
    else:
        dataset_image_cv = brighter(dataset_image_cv, random.uniform(1.01,1.10))
