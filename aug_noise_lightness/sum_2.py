...
def handle_img(rootpath,savepath,classes,classes_dict,img,img_xml,w,h,type):
    ...
    if type==14:
        clas = cla
        im_flip, coords=tl.prepro.obj_box_horizontal_flip(image, coords=ann, is_rescale=True, is_center=True, is_random=False)
...
if __name__=='__main__':
 ...
 ###7.自适应亮度增强 8.随机遮挡  9.图像高斯模糊 10.压缩图像#####
 ...
 ###15:图像任意旋转，传入旋转角度列表anglelist=[90,-90]#####
    type=4
    rootpath = r"/media/htbao/xunfei/数据集制作/已制作数据集/xunfei_yolo/train/images"
    savepath = r"./augmentation"
    #type 6 去除高光需要给mask蒙版的保存地址
    masksavepath = r"./mask"
    #type 11-15 需要传入 file_path:传入类别的信息txt，最好和生成labelmap的顺序一致
    file_path = r"/media/htbao/xunfei/数据集制作/已制作数据集/xunfei_yolo/train/labels"
    #1.直方图均衡化 2.clahe自适应对比度直方图均衡化 3.白平衡 4.亮度增强 5.亮度，饱和度，对比度增强 6.去除图像上的高光部分
